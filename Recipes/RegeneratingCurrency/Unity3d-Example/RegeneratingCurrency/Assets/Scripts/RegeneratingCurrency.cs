...
public class RegeneratingCurrency : MonoBehaviour {
	...
	void Update () {
		...
		this.livesRegen.text = string.Format("Next life in: {0} seconds", Mathf.FloorToInt((float)(nextFreeTicket - DateTime.Now).TotalSeconds));
	}
	
	void SetUI(int tickets, int items, DateTime nextFree)
	{
		this.livesValue.text = string.Format("{0}", tickets);
		this.livesRegen.text = string.Format("{0}", items);
		this.gemsValue.text = string.Format("{0}", nextFree);
	}
	...
	void GetInventory()
	{
		PlayFabClientAPI.ProcessApiCallback<GetUserInventoryResult> OnGetInventoryResult = (GetUserInventoryResult result) => {
			...
			{
				nextFreeTicket = rechargeDetails.RechargeTime;
				Debug.Log(string.Format("Your next free ticket will arrive at: {0}", nextFreeTicket));
				this.livesRegen.text = string.Format("Next life in: {0} seconds", (nextFreeTicket - DateTime.Now).TotalSeconds);
			}
