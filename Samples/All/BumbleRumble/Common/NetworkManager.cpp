...
void NetworkManager::CreateAndConnectToNetwork(std::vector<std::string>& playerIds, std::function<void(std::string)> callback)
{
	...
	PartyRegion localRegion{
		"westus",
		0
	};

	std::vector<PartyRegion> regions;
	regions.push_back(localRegion);
	...
	// Setup the network invitation configuration to allow specific players to join
	PartyInvitationConfiguration invitationConfiguration{
		m_networkId,                                 // invitation identifier
		PartyInvitationRevocability::Anyone,         // revokability
		static_cast<uint32_t>(additionalIds.size()), // authorized user count
		additionalIds.data()                         // authorized user list
	};
	...
	PartyManager::GetSingleton().CreateNewNetwork(
		...
		&cfg,                                       // Network Config
		static_cast<uint32_t>(regions.size()),      // Region List Count
