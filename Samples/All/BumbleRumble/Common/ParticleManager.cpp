    ...
    Particles(count)
{
    for (int i = 0; i < count; i++)
    {
        ...
        m_freeParticles.push(Particles[i]);
    }
}
...
{
    m_freeParticles = std::queue<std::shared_ptr<Particle>>();
...
void ParticleSystem::Initialize()
{
    ...
    // create the cache
    m_particles = std::make_unique<ParticleCache>(ParticleCount);

    // load the texture
    try
    {
        m_texture = Managers::Get<ContentManager>()->LoadTexture(TextureName.c_str());
    }
    catch (std::exception)
    {
        m_texture = Managers::Get<ContentManager>()->LoadTexture("Assets\\Textures\\Particles\\defaultParticle.png");
    }

    // calculate the origin on the texture
    TextureOrigin = SimpleMath::Vector2(static_cast<float>(m_texture->Width()) / 2.0f, static_cast<float>(m_texture->Height()) / 2.0f);
...
{
    // if the system isn't m_active, dont' update at all
    if (!IsActive())
    {
        return;
    }
    ...
    // update the m_active flag, based on if there are any used particles
    m_active = m_particles->UsedCount() > 0;
}
...
void ParticleSystem::Draw(std::shared_ptr<RenderContext> renderContext)
{
    ...
    // draw each particle
    for (auto &particle : m_particles->Particles)
    {
...
void ParticleSystem::GenerateParticles(float elapsedTime)
{
    ...
    while (ReleaseTimer >= ReleaseRate)
    {
        ...
        auto particle = m_particles->GetNextParticle();
        if (particle == nullptr)
        {
            break;
        }
...
{
    for (size_t i = 0; i < m_particles->Particles.size(); ++i)
    {
        if (m_particles->Particles[i]->TimeRemaining > 0.0f)
        {
            // update the timer on the particle
            m_particles->Particles[i]->TimeRemaining -= elapsedTime;

            // if the particle just timed out on this update, 
            // add it to the freed-list.
            if (m_particles->Particles[i]->TimeRemaining <= 0.0f)
            {
                m_particles->ReleaseParticle(m_particles->Particles[i]);
                continue;
            }

            // update the particle
            m_particles->Particles[i]->Update(
                elapsedTime,
                AngularVelocity,
                ScaleDeltaPerSecond,
...
std::shared_ptr<ParticleEffect> ParticleEffectManager::CreateLaserExplosionEffect()
{
...

    {
        ...
        system->TextureName = "Assets\\Textures\\Particles\\smoke.png";
...
std::shared_ptr<ParticleEffect> ParticleEffectManager::CreateMineExplosionEffect()
{
...

    {
        ...
        system->Duration = 2;
        system->InitialDelay = 0;
        system->TextureName = "Assets\\Textures\\Particles\\particle.png";
...
std::shared_ptr<ParticleEffect> ParticleEffectManager::CreateRocketTrailEffect()
{
    ...
    }
    {
        ...
        system->Duration = 3;
        system->InitialDelay = 0;
        system->TextureName = "Assets\\Textures\\Particles\\spark.png";
        system->BlendMode = SpriteBlendMode::Additive;
        system->Position = SimpleMath::Vector2(0, 0);
        system->Color.f[0] = 1.0f;
        system->Color.f[1] = 1.0f;
