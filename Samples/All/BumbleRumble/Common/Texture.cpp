    ...
    if (_wcsicmp(ext, L".dds") == 0)
    {
        ThrowIfFailed(
            CreateDDSTextureFromFileEx(device, resourceUpload, fileName, 0,
                D3D12_RESOURCE_FLAG_NONE, forceSRGB ? DDS_LOADER_FORCE_SRGB : 0,
    ...
    else
    {
        ThrowIfFailed(
            CreateWICTextureFromFileEx(device, resourceUpload, fileName, 0,
                D3D12_RESOURCE_FLAG_NONE, forceSRGB ? WIC_LOADER_FORCE_SRGB : 0,
...
_Use_decl_annotations_
Texture::Texture(ID3D11Device* device, const char* fileName, bool forceSRGB) :
    m_width(0),
    m_height(0),
    m_depth(0),
    m_mips(0),
    ...
    m_alphaMode(DDS_ALPHA_MODE_UNKNOWN)
{
    ...
    std::wstring wfileName = BumbleRumble::StrToWStr(fileName);

    WCHAR ext[_MAX_EXT];
    _wsplitpath_s(wfileName.c_str(), nullptr, 0, nullptr, 0, nullptr, 0, ext, _MAX_EXT);
    ...
    {
        ThrowIfFailed(
            CreateDDSTextureFromFileEx(device, wfileName.c_str(), 0,
    ...
    {
        ThrowIfFailed(
            CreateWICTextureFromFileEx(device, wfileName.c_str(), 0, 
                D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, 0,
