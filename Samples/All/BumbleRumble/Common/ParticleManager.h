...
public:
    ...
    inline int FreeCount() const { return static_cast<int>(m_freeParticles.size()); }
    inline int UsedCount() const { return static_cast<int>(Particles.size() - (int)m_freeParticles.size()); }

    std::vector<std::shared_ptr<Particle>> Particles;

private:
    std::queue<std::shared_ptr<Particle>> m_freeParticles;
};
...
public:
    ...
    inline bool IsActive() const { return m_active || TimeRemaining > 0.0f; }
...
private:
    ...
    std::unique_ptr<ParticleCache> m_particles;
};
